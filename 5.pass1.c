...
{
	int i,j;
	int c=value,x,digits=0,sum=0,mult;
	while(c!=0)
	{
		c=c/10;
		digits+=1;	
...
void main()
{
	...
	if(strcmp(opcode,"START")==0)//if opcode is start
	{
		...
		locctr=hextoint(atoi(start));//initialize locctr to starting address
	...
	fprintf(flen,"%d",locctr-hextoint(atoi(start)));//save locctr-start as program length
	fclose(flen);
	fclose(fip);
	fclose(fin);
